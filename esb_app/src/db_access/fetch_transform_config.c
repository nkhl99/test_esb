 ...
 * sql query:SELECT config_key,config_value, FROM transform_config 
 * WHERE route_id = ?
...
{  
   ...
   */
    ...
    transform_t *config;
    config = (transform_t *)malloc(sizeof(transform_t));
    ...
    {
        printf("No transform_config exists for route_id.");
    }
...
{
    transform_t* t = fetch_transform_config(1);
